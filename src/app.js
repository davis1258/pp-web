...
async function getToken() {
    window.open("http://localhost:3000/renderFast")
    // try {

    //     const response = await fetch("/api/renderFast", {
    //         method: "GET",
    //         headers: {
    ...
    //     window.open("checkout.html?clientToken=" + data.clientToken+"&clientId="+data.clientId);
    //     return data;
    // } catch (error) {
...
async function createOrderSaveCards() {
    resultMessage("");
    try {
        const response = await fetch("/api/saveCardsOrders", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            ...
            body: JSON.stringify({
                card: {
                    ...
                    expiry: document.getElementById(
        ...
        if (orderData.code=='0000') {
            alert("successfully and the customer id is "+orderData.data);
            payCard.close();
        } else {
            const errorMessage = orderData.msg;
            alert("fail ,errr: "+orderData.msg);
...
async function payByCustomerId() {
    resultMessage("");
    try {
        const response = await fetch("/api/payWithVaultId", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            ...
            // like product ids and quantities
            body: JSON.stringify({
                vaultId: document.getElementById(
                        "customer-id"
                    ).value
            }),
        });
        ...
        console.error(orderData);
        if (orderData.code=='0000') {
            alert("successfully and the order id is "+orderData.data);
            payCustomerId.close();
        } else {
            const errorMessage = orderData.msg;
            alert("fail ,errr: "+orderData.msg);
